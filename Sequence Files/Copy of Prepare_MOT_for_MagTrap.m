...
setDigitalChannel(calctime(curtime,-500),11,0);
%% Compression
    ...
    if D1_cMOT
       ...
       setAnalogChannel(curtime,'K Repump FM',0,2);
        ...
        if (seqdata.atomtype==1 || seqdata.atomtype==4)%K-40
            ...
            k_cMOT_detuning = 5; %20 Oct 30, 2015 20; 
            ...
            %turn repump down
            setAnalogChannel(calctime(curtime,cMOT_time - k_cMOT_time),'K Repump AM',0.25); 
            %turn trap down
            setAnalogChannel(calctime(curtime,cMOT_time - k_cMOT_time),'K Trap AM',0.70); %0.3 Oct 30, 2015
            
            %increase gradient to 15 G/cm
            %setAnalogChannel(calctime(curtime,cMOT_time - k_cMOT_time),'MOT Coil',15);
...
%             addOutputParam('rb_cMOT_detuning',rb_cMOT_detuning);   
            ...
            %turn repump down
            setAnalogChannel(calctime(curtime,cMOT_time - rb_cMOT_time),'Rb Repump AM',0.25);
            %before 2016-11-25 0.2%0.2 June 25

            %turn trap down  (cannot change Trap Power very well due to AOM placement before TA)
%           setAnalogChannel(calctime(curtime,cMOT_time - rb_cMOT_time),'Rb Trap AM',0.3);
...
do_molasses = 1 && (seqdata.flags.image_type ~= 4);
 
molasses_time = 10; %(10 for Rb evap ) %10 (overwritten below if K is selected)

%digital trigger

setDigitalChannel(calctime(curtime,-2),'Rb Repump Imaging',1); %1 = open, 0 = closed

if do_molasses
    
    k_molasses_detuning_list = 5.5; %%20 Oct 30, 2015
    ...
    %turn on the Y (quantizing) shim 
    setAnalogChannel(calctime(curtime,-1),'Y Shim',0.25); %0.25 for D2, 0.25 for D1
    %turn on the X (left/right) shim 
    setAnalogChannel(calctime(curtime,-1),'X Shim',0.2); %0.2 for D2, 0.4 for D1
    %turn on the Z (top/bottom) shim 
    setAnalogChannel(calctime(curtime,-1),'Z Shim',0.05); %0.11 for D2, 0 for D1
                                                             
    %set shims and detuning (atom dependent)
    if (seqdata.atomtype==1 || seqdata.atomtype==4)%K40
        molasses_time = 7.5; %7.5
...
%         kmolasses_am=getScanParameter(kmolasses_List,seqdata.scancycle,seqdata.randcyclelist,'kmolasses_am');
        setAnalogChannel(calctime(curtime,0),'K Repump AM',0.5); %0.5
    end
    ...
    if (seqdata.atomtype==3 || seqdata.atomtype==4) %Rb
        ...
        setAnalogChannel(curtime,'Rb Trap AM',0.7); %0.7
        %turn repump down
%         rbmolasses_List=[0.06];
%         rbmolasses_am=getScanParameter(rbmolasses_List,seqdata.scancycle,seqdata.randcyclelist,'rbmolasses_am');
        rb_molasses_repump_power = 0.03;
        setAnalogChannel(curtime,'Rb Repump AM',rb_molasses_repump_power); %0.14 
        addOutputParam('rb_molasses_repump_power',rb_molasses_repump_power)
...
% % % % 
% % % %  K_Grey_molasses_time = 1;
% % % %  
% % % %  do_K_D2_Grey_molasses = 0;
% % % %  
% % % %  if do_K_D2_Grey_molasses
% % % %                     
% % % %  K_molasses_repump_detuning = 0.01;  %in MHZ
% % % %  K_molasses_trap_detuning = 0.01;
% % % %   
% % % % % %Parameters
% % % %         %K_repump double pass AOM- Rigol Channel 1
% % % %         K_molasses_repump_freq = 78.1 + K_molasses_repump_detuning; %This is multiplied by 4 using two doublers %83.71
% % % %         K_molasses_repump_amp = 0.60;
% % % %         K_molasses_repump_offset = 0;
% % % %         
% % % %         %K_repump shear mode AOM path
% % % %         setAnalogChannel(curtime,'K Repump 0th Shutter',0); % turn off K repump shear mode 0th power 
...
% % % %         K_molasses_trap_freq = 127.1 + K_molasses_trap_detuning;
% % % %         K_molasses_trap_amp = 1.250;
...
% % % %         
% % % %         setAnalogChannel(curtime,'Grey Molasses switch',5)  %0: MOT Beam sources 5: D2 Molsses Beam sources
% % % % 
% % % %         curtime = calctime(curtime,K_Grey_molasses_time);
% % % %         
% % % %         setAnalogChannel(curtime,'Grey Molasses switch',0) % turn off K D2
...
% % % % % % % % Turn off Rb Trap beam
% % % % % % %     setDigitalChannel(curtime,'Rb Trap Shutter',0); % Rb trap shutter off;
% % % % % % %     setDigitalChannel(curtime,'Rb Trap TTL',1); % Rb trap AOM Off;
% % % % % % %     setAnalogChannel(curtime,'Rb Trap AM',0);  % Set Rb trap AOM power to 0;
% % % % % % % % Turn off Rb Repump beam
% % % % % % %     setDigitalChannel(curtime,'Rb Repump Shutter',0); % Rb repump shutter off;
% % % % % % % %     setDigitalChannel(curtime,'Rb Repump AM',0); % Rb repump AOM off; no TTL for this channel
% % % % % % %     setAnalogChannel(curtime,'Rb Repump AM',0);  % Set Rb repump AOM power to 0;
% % % % % % % % Turn off K Trap beam
% % % % % % %     setDigitalChannel(curtime,'',0); % K Trap shutter off;
% % % % % % %     setDigitalChannel(curtime,'',1); % K Trap AOM Off;
% % % % % % %     setAnalogChannel(curtime);  % Set K Trap AOM power to 0;
% % % % % % % % Turn off K Repump beam
% % % % % % %     setDigitalChannel(curtime); % K Repump shutter off;
% % % % % % %     setDigitalChannel(curtime); % K Repump AOM off;
% % % % % % %     setAnalogChannel(curtime);  % set K repump AOM power to 0;
% % % % % % % 

%modify the "Turn the trap light off" @ 20180316, the previous codes is as shown following
do_D1_molasses = 0;

if ( seqdata.flags.image_type ~= 4 && do_D1_molasses == 0)
    if do_molasses
        turn_off_beam(calctime(curtime,0),1);    % why do not turn off Repump? for optical pumping? 20180316
    else
...
do_optical_pumping = 1 && (seqdata.flags.image_type ~= 4);
...
%% Turn off the repump
if ( seqdata.flags.image_type ~= 4 )
    turn_off_beam(calctime(curtime,0.5),2); %a little delayed w.r.t trap
end
