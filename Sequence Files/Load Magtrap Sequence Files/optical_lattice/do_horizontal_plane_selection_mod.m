...
opt = struct('Offset_Field',4, ... field (in G) to horizontally offset the QP gradient
             ...
             'Raman_Power', 0, ... Voltage sent to Raman AOMS.
...
field_shift_settle = 80;                % settling time after initial and final field shifts
...
%Use pulse-shaped Raman transfer instead of microwaves for horizontal plane selection.        

Pulse_Time = opt.Microwave_Pulse_Length;
Rise_Time = 0;
...
setAnalogChannel(calctime(curtime,-150-Rise_Time),'Raman VVA',0);
...
%         DigitalPulse(calctime(curtime,F_Pump_Time),'D1 TTL B',10,1);
...
elseif (opt.Microwave_Or_Raman == 2)
    ...
    if strcmp(opt.Rigol_Mode, 'Sweep')
        str = sprintf('SOURce1:SWEep:STATe ON;SOURce1:SWEep:TRIGger:SOURce EXTernal;SOURce1:SWEep:TIME %gMS;SOURce1:FREQuency:CENTer %gMHZ;SOURce1:FREQuency:SPAN %gMHZ;SOURce1:VOLT %g;SOURce2:VOLT %g;', opt.Modulation_Time, opt.Raman_AOM_Frequency, opt.Selection_Range, opt.Raman_Power, opt.Raman_Power);
    elseif strcmp(opt.Rigol_Mode, 'Pulse')
        str = sprintf('SOURce1:SWEep:STATe OFF;SOURce1:MOD:STATe OFF;SOURce1:FREQuency %gMHZ;SOURce1:VOLT %g;SOURce2:VOLT %g;', opt.Raman_AOM_Frequency, opt.Raman_Power, opt.Raman_Power);
    elseif strcmp(opt.Rigol_Mode, 'Modulate')
        str = sprintf('SOURce1:MOD:STATe ON;SOURce1:MOD:TYPe FM;SOURce1:MOD:FM:INTernal:FUNCtion TRIangle;SOURce1:MOD:FM:INTernal:FREQuency %gKHZ;SOURce1:FREQuency %gMHZ;SOURce1:MOD:FM:DEViation %gMHZ;SOURce1:VOLT %g;SOURce2:VOLT %g;', 1/opt.Modulation_Time, opt.Raman_AOM_Frequency, abs(opt.Selection_Range), opt.Raman_Power, opt.Raman_Power);            
    end
...
%             DigitalPulse(calctime(curtime,-10),'D1 OP TTL',10,0);
    ...
    ScopeTriggerPulse(curtime,'Raman Plane Select');
    Rise_Time = 0.1;
    Pulse_Time = opt.Microwave_Pulse_Length - 2*Rise_Time;
    ...
    AnalogFuncTo(calctime(curtime,0-Rise_Time),'Raman VVA',@(t,tt,y1,y2)(ramp_minjerk(t,tt,y1,y2)),Rise_Time,Rise_Time,9.9);
    curtime = AnalogFuncTo(calctime(curtime,Pulse_Time),'Raman VVA',@(t,tt,y1,y2)(ramp_minjerk(t,tt,y1,y2)),Rise_Time,Rise_Time,0);        

    setAnalogChannel(calctime(curtime,50),'Raman VVA',10); 
%     ScopeTriggerPulse(curtime,'Raman Plane Select');
...
if opt.Resonant_Light_Removal
    ...
    %Set shims to this angle for killing.
    Kill_Angle = -90;
    
    %Determine how much to turn on the X and Y shims to get this frequency
    %shift at the requested angle
    X_Shim_Value_Kill = frequency_shift * cosd(Kill_Angle) / Shim_Calibration_Values(1);
    Y_Shim_Value_Kill = frequency_shift * sind(Kill_Angle) / Shim_Calibration_Values(2);

    %Ramp shim to start value before generator turns on
    clear('ramp');
    ramp.shim_ramptime = field_shift_time;
    ramp.shim_ramp_delay = 0; %offset from the beginning of uwave pulse
    ramp.xshim_final = X_Shim_Value_Kill;
    ramp.yshim_final = Y_Shim_Value_Kill;
    ramp.settling_time = 0;
    ramp.fesh_ramptime = 0;
    ramp.QP_ramptime = 0;
