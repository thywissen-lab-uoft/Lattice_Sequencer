%------
%Author: DM
%Created: July 2011
%Summary: This function takes a fluorescence image of the MOT. NOTE: Can
%only be used when the MOT is already on
%------

function timeout=Blue_MOT_fluor_probe(timein,No_MOT)

global seqdata;

curtime = timein;

wait_time = 1;

%% FLUORESCENCE IMAGING

if No_MOT ==1
      
%shut off MOT gradient (leave beams on)
setAnalogChannel(calctime(curtime,100),8,0);
       
end



%% Take 1st probe picture

pulse_length = 1.0;

%setAnalogChannel(calctime(curtime,0),25,0.4);

curtime = calctime(curtime, wait_time);




%Camera trigger (Note: absorption length is set by exposure setting in
%camera program)
DigitalPulse(curtime,1,pulse_length,1);


%% Take 2nd probe picture after 1s 
% 
% %shut off MOT gradient (leave beams on)
% setAnalogChannel(calctime(curtime,100),8,0);
% 
curtime = calctime(curtime, wait_time);

%100us Camera trigger
curtime = DigitalPulse(calctime(curtime,1000),1,pulse_length,1);
% 



% %% Turn Y shim off
% setAnalogChannel(curtime,19,0.00);
% 
% %turn off blue MOT (if red MOT is on it will just get turned back on so
% %don't worry)
% 
% if blue_mot
%     setDigitalChannel(calctime(curtime,100),23,0);
% end
% 
timeout=curtime;

end
