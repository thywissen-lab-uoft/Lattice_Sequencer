...
% THIS CODE IS UGLY AND CONFUSING, NEEDS TO BE FIXED
 function curtime = ac_conductivity(curtime,freq,field,evap_depth,pulse_depth,mod_strength,mod_ramp_time,Nplane_shift,lattice_load_depth,hold_time)
        global seqdata;        
        ...
        seqdata.flags.conductivity_cross_thermalize     = 1; % Wait after modulating
...
% freq_list = [20 30 40 50 60 65 70 72 75 78 80 85 90 100 110 120 130];
freq_list = [70];
...
% Lattice pulse depth
pulse_list = [0];[6 5.5];%tbd [6.5 6 5 5 5 4.5 4.5];
...
% evaporation depths
power_conductivity_list = [0.057]; [0.0637];
...
%     mod_strength_list = calc_drive(2,Gamma_list(bb),0.8,freq_list);
...
%         mod_strength = min([mod_strength 4]);
        ...
        out.SequenceFunctions   = {@main_settings,@(curtime) ...
            ac_conductivity(curtime,f,B,power_conductivity,pulse_depth,mod_strength,mod_ramp_time,Nplane,lattice_load_depth,thold),@main_sequence};
