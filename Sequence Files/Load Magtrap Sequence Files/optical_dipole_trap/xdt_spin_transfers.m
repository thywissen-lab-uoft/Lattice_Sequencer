...
seqdata.flags.xdt_spin_xfer_ramp_field             = 0;
seqdata.flags.xdt_spin_xfer_transfer_Rb            = 0;
seqdata.flags.xdt_spin_xfer_transfer_Rb_freq_sweep = 1;

seqdata.flags.xdt_spin_xfer_transfer_K             = 0;
seqdata.flags.xdt_spin_xfer_Rb_2_kill              = 0;
seqdata.flags.xdt_spin_xfer_Rb_2_kill2             = 0;
...
% New Try
defVar('xdt_spin_field_1',[19.5]);19.4;
defVar('xdt_spin_field_delta',[.2]);
...
curtime = calctime(curtime,60);
    ...
    % Prepare
    setDigitalChannel(calctime(curtime,-30),'Rb uWave TTL',0);           % RF OFF
    setDigitalChannel(calctime(curtime,-30),'Rb uWave SRS TTL',0);       % RF OFF
    setDigitalChannel(calctime(curtime,-20),'RF/uWave Transfer',1);      % 0: RF, 1: uwave      
    setDigitalChannel(calctime(curtime,-20),'K/Rb uWave Transfer',1);    % 0: K, 1: Rb     
    setDigitalChannel(calctime(curtime,-20),'Rb Source Transfer',0);     % 0 = Anritsu, 1 = Sextupler  
 
    % uWave sweep settings
    
    defVar('Rb_uWave_transfer_power',9,'dBm');
    defVar('Rb_uWave_transfer_freq',6875.6,'MHz');
    defVar('Rb_uWave_transfer_freq_amp',50,'kHz');
    defVar('Rb_uWave_transfer_time',[10],'ms');
    
    sweep_time = getVar('Rb_uWave_transfer_time');
    ...
    % Turn on the uWave        
    setDigitalChannel(calctime(curtime,0),'Rb uWave SRS TTL',1);  
...
curtime = setAnalogChannel(calctime(curtime,50),'uWave FM/AM',-1);    % Reset the uWave deviation after a while
