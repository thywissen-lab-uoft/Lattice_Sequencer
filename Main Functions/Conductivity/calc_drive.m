function [displacement] = calc_drive(Tpred,Gpred,amp_desired,freq_list)
    %% Constants
    h = 6.626e-34;
    hbar = h/(2*pi);
    kb = 1.380649e-23;
...
%     tunneling = 563.4109123332288;
    ...
    f = freq_list;
    for ff = 1:length(f)
        d(ff) = hbar*2*pi*f(ff)*amp_desired/(aLatt^2*m*wXDT^2*sqrt(qfit_real(T,G,2*pi*f(ff))^2+qfit_imag(T,G,2*pi*f(ff))^2))/v2um; %V
    end
