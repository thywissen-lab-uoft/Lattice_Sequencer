function var_new = stripe_feedback2

var_new = NaN;

% Settings for feedback
varName = 'qgm_plane_uwave_frequency_offset';
flagName = 'qgm_stripe_feedback2';
N = 50;
n = 3;
...
%% Collect Data that has flag enabled
% Get most recent data
data = getRecentGuiData(N);

doKeep = zeros(length(data),1);
for ii = 1:length(data)
    if isfield(data{ii},'Flags') && ...
            isfield(data{ii}.Flags,flagName) && ...
            data{ii}.Flags.(flagName)
        doKeep(ii) = 1;
    end       
end
data = data(doKeep);
...
for n = 1:length(data)
    if ~isfield(data{n},'StripeFit')
        continue
    end
    
    timeVec(end+1) = datenum(data{n}.Date);
    varVec(end+1) = data{n}.Params.(varName);
    thetaVec(end+1) = data{n}.StripeFit(ind).theta;
    thetaErrVec(end+1) = data{n}.StripeFit(ind).theta_err;
    LVec(end+1) = data{n}.StripeFit(ind).L;
    LErrVec(end+1) = data{n}.StripeFit(ind).L_err;
    phiVec(end+1) = data{n}.StripeFit(ind).phi;
    phiErrVec(end+1) = data{n}.StripeFit(ind).phi_err;
    BVec(end+1) = data{n}.StripeFit(ind).B;
    BErrVec(end+1) = data{n}.StripeFit(ind).B_err;
end
...
function calculatePhaseError(data)
