...
function timeout = transport_test_2(timein)
...
%% Testing Coil 16 in QP Mode

% Wait
curtime = calctime(curtime,100);


setDigitalChannel(curtime,'Kitten Relay',0); % Kitten Relay off 0: OFF, 1: ON
setDigitalChannel(curtime,'15/16 Switch',1); % Turn on 15/16 Switch 0: OFF, 1: ON
setDigitalChannel(curtime,'Coil 16 TTL',0);  % Coil 16 on (obsolete?) 1: turns coil off; 0: coil can be on
...
%set FF
setAnalogChannel(calctime(curtime,-200),'Transport FF',25*(abs(current)/30)/3 + 0.5);
