...
%------
function y = transport_coil_currents_kitten_troubleshoot_volt_test(time,position,flag)
...
transportfolder = 'transportsplines\';
coilone = dlmread([transportfolder 'rev44coilone.txt'],',',0,1)*vertical_scale;
coiltwo = dlmread([transportfolder 'rev44coiltwo.txt'],',',0,1)*vertical_scale;
...
coil_offset = [0 30 41 43 85 116 148 179 212 242 274 338 350 365 365 365 365 365 365 365 365 365]; %338
...
coil_resistance = [0 375 418 100 100 100 100 100 100 100 100 100 100 188 173 173 173 310 310 310];


%max FET power (to check we don't exceed this...especially important for the bridges)
%Make sure to update if something changes
max_fet_power = [0 700 5000 5000 700 700 700 700 700 700 700 5000 5000 700 208 208 208 700 700 700]; 
...
coil_range(1,13) = 250;
coil_range(2,13) = 380;
coil_scale_factors(13) = 0.96; %0.95
coil_offset(13) = 0;
coil_widths(13) = 0.99;
...
transport_channels = [18 7:17 9 22:24 20:21 1 3 -22 -28]; %CHANGE TO 7:17;
    ...
    if (~flag && i<=num_analog_channels)
        ...
        if (transport_channels(i)~=18 && transport_channels(i)~=3) %skip the voltage and kitten channels
            ...
            if sum(((abs(currentarray(channel_indices,3)).*voltages-currentarray(channel_indices,3).^2*coil_resistance(i)/1000).*(currentarray(channel_indices,3)~=nullval))>max_fet_power(i))
                error(['FET Power exceeded for channel:' num2str(transport_channels(i))]);
            end
    ...
    function y = channel_current(channel,pos,offset,width,coilrange)
        ...
        switch channel
            ...
            case 1  %This is the FF channel
                 ...
                 horizontalvoltage = 9.0*overallscale; %9/6.6
...
%                
            ...
            case 14 %1st vert transport coil
            ...
            case 15 %2nd vert transport coil
            ...
            case 16 %3rd vert transport coil
            ...
            case 17 %4th vert transport coil
            ...
            case 18 %bottom QP coil lower FET
            ...
            case 20 %kitten FET
                ...
                y(ind) = y(ind) + 50.0*(ppval(pp1,x)>=k_turn_on_point); %some very large current
                
                %prevent rippling at the beginning
                y(ind) = y(ind) + nullval.*(y(ind)==0).*(x<=450);
                
                y(ind) = y(ind) + (ppval(pp2,x)+ppval(pp1,x)).*(ppval(pp1,x)<k_turn_on_point).*(x>450).*(x<532);
                
                %y(ind) = y(ind) + ((ppval(pp2,532)+ppval(pp1,532)).*(ppval(pp1,532)<k_turn_on_point).*(1-x/(539-532))+6.2).*(x>=532);
                
                end
            
                
            case 21 %15/16 TTL switch
...
%  index=seqdata.randcyclelist(seqdata.cycle);
% %  
% turn_on_point =  turn_on_point_list(index)
%   addOutputParam('hor_transport_distance', turn_on_point);
            ...
            case 22 %new coil relay
                 
                y(ind) = y(ind) + (x>210) - (x<=210);
