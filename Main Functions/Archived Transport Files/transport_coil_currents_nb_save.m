...
%------
function y = transport_coil_currents(time,position,flag)
...
%horizontal current curves
    hor_current_rev = 0; %0: rev0, 1: rev1
...
if hor_current_rev == 0
%coil_offset = [0 30 41 43 85 116 148 179 212 242 274 338 360 360 360 360 360 360 360]; %the peak of each coil in position (mm)
coil_offset = [0 30 41 43 85 116 148 179 212 242 274 338 363 360 357 360 360 360 360]; 
%coil_offset = [0 30 41 43 85 116 148 179 212 242 274 338 360 360 358 360 359 361 360]; rev1 
end
%coil_offset = [0 30 41 43 85 116 148 179 212 242 274 338 360 360 358 361 358 363 360]; 


if hor_current_rev == 1
%coil_offset = [0 0 0 0 85 116 148 179 212 242 274 338 360 361 358 360 358 363 360]; 
end

if use_splines
    coil_offset(12:end) = 0; 
end
...
%MOT Coil range
coil_range(1,3) = 0;
coil_range(2,3) = 361; %75 for non-divergent curves
...
coil_range(1,14) = 260; 
coil_range(2,14) = 460; %460
coil_scale_factors(14) = -0.9; %-0.85*1.1  *rev1* -1.0  *rev2* -1.0 SC: -0.85*1.4
...
if ~use_splines
    coil_scale_factors(16) = 1.4*verticalscale; %1.4  *rev1*1.3  *rev2* 1.0 SC: 1.4
end
    ...
    function y = channel_current(channel,pos,offset,width,coilrange)
        ...
        switch channel
            ...
            case 2 %push    
                ...
                %**********rev1 currents
                if hor_current_rev == 1
%                   y(ind) = y(ind) + (x<40).*(x>0).*...
%                         (-2.568791393403187 + 2.123485627141295*x - 0.13560267911680962*x.^2 + 0.00829612246112741*x.^3 -...
%                         0.00011513540340325809*x.^4);
% 
%                         
%                   y(ind) = y(ind) + (x<=44).*(x>=40).*...
%                         (105.72424216568336- 10.534853374259935*(x-38) + 8.869760701220384*(x-38).^2 - 2.5827948746893195*(x-38).^3 + 0.009363009855242055*(x-38).^4 +...
%                         0.0921081357598494*(x-38).^5 - 0.011463756242282637*(x-38).^6);
%                     
%                   y(ind) = y(ind);
                end
            ...
            case 3 %MOT 
                ...
                %**********rev1 currents
                if hor_current_rev == 1
%                 y(ind) = y(ind) + (x<44).*(x>=0).*...
%                         (17.568438645963024+ 1.0264034105884532*x - 0.23013121434658876*x.^2 + 0.021888155762724362*x.^3 - 0.0010122548586652025*x.^4 +...
%                         0.000022270148484135257*x.^5 - 1.8633292148379106E-7*x.^6);
%             
%                 y(ind) = y(ind) + (x<68).*(x>=44).*...
%                         (8.637228495910419- 0.1330999618178875*(x-44) - 0.024061302378259472*(x-44).^2 + 0.000605519998146524*(x-44).^3);
                end
               ...
               case 4 %1st transport coil
                ...
                %**********rev1 currents
                 if hor_current_rev == 1              
%                 y(ind) = y(ind) + (x<44).*(x>0).*...
%                         (.8441985989808058+ 4.852356499799834*x - 0.07890845899401586*x.^2 - 0.011129093402750406*x.^3 +...
%                         0.0007070251294060486*x.^4 - 0.000016323667912964647*x.^5 + 1.3714134724172376E-7*x.^6);
%             
%                 y(ind) = y(ind) + (x<103).*(x>=44).*...
%                         (68.7195862624008- 0.41791113155436005*(x-44) - 0.012004114160145673*(x-44).^2 - 0.0004188536270858176*(x-44).^3 + 6.876786423651612E-6*(x-44).^4);
             end
            ...
            case 5 %2nd transport coil
                ...
                %**********rev1 currents
                if hor_current_rev == 1
%                 y(ind) = y(ind) + (x<84).*(x>=44).*...
%                      (-1.4735141075088714 + 1.7151111935614263*(x-44) - 0.017740107613947058*(x-44).^2 - 0.00004425423501411746*(x-44).^3);
%                    
%                 y(ind) = y(ind) + (x<133).*(x>=84).*...
%                     (34.61781522830108 + 0.4956517274041638*(x-82) - 0.05188903433434508*(x-82).^2 + 0.0005602306781839571*(x-82).^3);
                end
            ...
            case 13 %1st vert transport coil
                ...
                if use_splines
                    pp1 = create_transport_splines_nb(12);
                    pp2 = create_transport_splines_nb(12.5);
                    
                    y(ind) = y(ind) + (x<=360).*ppval(pp1,x);
                    
%                     %ramp between the values from 360-->360.1
%                     y(ind) = y(ind) + (x>360).*(x<360.1).*...
%                         (ppval(pp1,360)+(ppval(pp2,360.1)-ppval(pp1,360)).*(x-360)/0.1);
                    
                    %y(ind) = y(ind) + (x>=360.1).*ppval(pp2,x);
                    y(ind) = y(ind) + (x>360).*ppval(pp2,x);
    ...
    %                     -(-13.838258411267354+0.05947239838899692*x-0.07100519794104734*x.^2+0.0018251714087331981*x.^3);
            ...
            case 14 %2nd vert transport coil
                ...
                if use_splines
                    pp1 = create_transport_splines_nb(13);
                    pp2 = create_transport_splines_nb(13.5);
    ...
    %                     
            ...
            case 15 %3rd vert transport coil
                 ...
                 if use_splines
                    ...
                    y(ind) = y(ind) + (x<=360)*0;
                    y(ind) = y(ind) + (x>360).*(x<360.1).*((x-360)/0.1*ppval(pp,360.1));
                    y(ind) = y(ind) + (x>=360.1).*ppval(pp,x);
    ...
    %                         0.000012469136759791687*(x-135).^5-8.292190209162253E-8*(x-135).^6);
            ...
            case 19 %kitten
                ...
                if use_splines
                    ...
                    y(ind) = y(ind) + (ppval(pp1,x)<=0).*(ppval(pp1,x)+ppval(pp2,x));
                    
                    y(ind) = y(ind) + (ppval(pp1,x)>0).*60;
