function J=job_peakcond_v_temp_evap
   
%% AC Sequence Modifier
 function curtime = ac_p_evap(curtime,freq,field,lattice_pulse_depth,evap_depth,mod_strength,mod_ramp_time,uwave_freq_amp)
        global seqdata;        
...
evap_depth_list = [0.064:0.0005:0.070];

% Randomize the evap depths
evap_depth_list = evap_depth_list(randperm(numel(evap_depth_list)));


for ii = 1:length(evap_depth_list)
    % Get the current modulation frequency
    power_conductivity = evap_depth_list(ii);   
       
    
    out = struct;   
    out.SequenceFunctions   = {@main_settings,@(curtime) ...
        ac_p_evap(curtime,f,B_conductivity,pulse_depth,power_conductivity,mod_strength,mod_ramp_time,uwave_freq_amp),@main_sequence};
...
%     npt.JobCompleteFcn      = @jobComplete;
    ...
    out.JobName             = [num2str(ii) ' shake, ' num2str(f) ' Hz,' ...
        num2str(B_conductivity) 'G,' num2str(pulse_depth) 'ER, ' num2str(1e3*power_conductivity) ' mW ' num2str(mod_strength) ' amp, ' ...
