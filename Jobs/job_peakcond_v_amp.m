function J=job_peakcond_v_amp
   
%% AC Sequnece Modifier
% THIS CODE IS UGLY AND CONFUSING, NEEDS TO BE FIXED
 function curtime = ac_pvAmp(curtime,freq,field,evap_depth,mod_strength,mod_ramp_time,uwave_freq_amp)
        global seqdata;        
...
mod_strength_list = [0.2 0.4 0.5 0.6 0.8];

% Randomize the lattice pulse depths
mod_strength_list = mod_strength_list(randperm(numel(mod_strength_list)));


for ii = 1:length(mod_strength_list)
    % Get the current modulation frequency
   mod_strength = mod_strength_list(ii);   
    
    out = struct;   
    out.SequenceFunctions   = {@main_settings,@(curtime) ...
        ac_pvAmp(curtime,f,B_conductivity,power_conductivity,mod_strength,mod_ramp_time,uwave_freq_amp),@main_sequence};
