...
seqdata.params. shim_zero = [(0.1585), (-0.0432), (-0.0865)];  %Bipolar shim values (x,y,z) which zero the ambient field (found using uWave spectroscopy on May 9th, 2014)
...
seqdata.params. plug_shims = [(seqdata.params. shim_zero(1)-1.000), (seqdata.params. shim_zero(2)+0.15-0.025), (seqdata.params. shim_zero(3)+0.100+0.15)];%.46 -1.7 0.022
...
hold_list = [500];
seqdata.params. molasses_time = getScanParameter(hold_list,seqdata.scancycle,seqdata.randcyclelist,'hold_list');%192.5;
...
%% Switches
    ...
    seqdata.params.tof = 10;   % 45 for rough alignment, 20 for K-D diffraction

    % Transport curves
    seqdata.flags.hor_transport_type = 1; %1 20180315
    %0: min jerk curves, 1: slow down in middle section curves, 2: none
    seqdata.flags.ver_transport_type = 3; % 3 20180315
    ...
    seqdata.flags.compress_QP = 1; % compress QP after transport %1 20180315
    ...
    rf_evap_speed = [0.7 1.6]; %[0.9 1] little improvement; [0.2 1.2] small clouds but fast
    ...
    scope_trigger = 'test1';   'conductivity_modulation'; %Start Fluorescence Capture
%     exclude_trigger = 'all';
...
%  (as the experiment warms up, selected plane tends to move up a bit)
    ...
    obj_piezo_V_List = [6.4];
    % 0.1V = 700 nm, m7ust be larger than 0. larger value means farther away from the window.
...
%% and initialize repump imaging.
    ...
    setAnalogChannel(calctime(curtime,0),'xLattice',-9.9,1);
    setAnalogChannel(calctime(curtime,0),'yLattice',-9.9,1);
    setAnalogChannel(calctime(curtime,0),'zLattice',-9.9,1);
    
    setDigitalChannel(calctime(curtime,0),'xLatticeOFF',1);
    setDigitalChannel(calctime(curtime,0),'yLatticeOFF',1);
    setDigitalChannel(calctime(curtime,0),50,1); %Z Lattice TTL (when connected)
...
%% Pulse on XDTs for 100ms and trigger pyKraken for measurement
    ...
    measure_XDT_pointing = 1;
    if (measure_XDT_pointing)
        DigitalPulse(calctime(curtime,10),20,1,1) % trigger pulse
        %pulse XDTs on
        ...
        %turn XDTs off
        setAnalogChannel(calctime(curtime,51),'dipoleTrap1',0,1);
        setAnalogChannel(calctime(curtime,51),'dipoleTrap2',0,1);
        setDigitalChannel(calctime(curtime,51),'XDT TTL',1);
    end
...
curtime = Prepare_MOT_for_MagTrap(curtime);
    ...
    %Open K Trap AOM once again to keep it, and TA, warm. Shutter still closed.
    setDigitalChannel(calctime(curtime,1000),'K Trap TTL',0);
    setAnalogChannel(calctime(curtime,1000),'K Trap AM',0.8);
...
%         %this worked well with 0.6 kitten
        freqs_1 = [ start_freq 28 20 12]*MHz; %7.5 %[ start_freq 28 20 12]*MHz before 2018-03-06
%         RF_gain_1 = [7 7 7 7]*(7)/9 * 0.75; %9 9 9 (5)/9*0.75
        RF_gain_1 = 1*[ 9 9 9 9]*(9)/9*1; %9 9 9 (5)/9*0.75
%         sweep_times_1 = [0.5*16000 8000 3000].*rf_evap_speed(1);
        sweep_times_1 = [ 14000 6000 2000].*rf_evap_speed(1);%[ 12000 5000 1500].*rf_evap_speed(1);%[ 14000 6000 2000].*rf_evap_speed(1); before 2017-05-02
        %sweep_times_1 = [16000 8000 3000].*rf_evap_speed(1); % %[17000 8000 3000]  [16000 8000 3000].*1*.95; Dec 7 2013, [16000 8000 3000].*1*.65; Seemed as good or better for Rb only evap
...
% curtime = calctime(curtime,700);%700
    ...
    %open plug shutter
    setDigitalChannel(calctime(curtime,plug_offset-10),'Plug Shutter',1);
...
%     Evaporate to 0.7MHz to load into ODT (0.8MHz to look at Rb)
    freqs_1b = [freqs_1(end)/MHz*1.0 4 0.8 2]*MHz; %0.28 %0.315 freqs_1b = [freqs_1(end)/MHz*0.8 4 0.8 2]*MHz 
%     RF_gain_1b = [4 4 1 1]; %[-4 -4 -7 -7] prior to June 1 2015
    RF_gain_1b = [4 4 1 1]; % 8 8 5 5
    sweep_times_1b = [2000 2000 10]*rf_evap_speed(2); %[3000 2500 10]*rf_evap_speed(2);

if seqdata.flags. evap_away_Rb_in_QP == 1
    %Evaporate lower to clear out the Rb, leaving only K in the XDT
    freqs_1b = [freqs_1(end)/MHz*0.8 4 0.35 2]*MHz;
...
%         setDigitalChannel(calctime(curtime, plug_offset+199),'Plug Mode Switch',1);
    setDigitalChannel(calctime(curtime,-10),'Plug Shutter',0);
...
if seqdata.flags.image_type == 0 % Absorption Image
    setDigitalChannel(calctime(curtime,0),12,1);% scope trigger TTL High
    setDigitalChannel(calctime(curtime,1),12,0);% scope trigger TTL Low
...
k_MOT_detuning_list = [18];% before 2018-02-14: 18
