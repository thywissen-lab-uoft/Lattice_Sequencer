...
dim ch as long
dim val,val_dig2,val_dig1,val_dig3 as long
dim tempval as long 
...
dim digitalactive,digital2active,digital3active as long
...
INIT:
  ...
  P2_DIGPROG(3,011111111111111111111111111111111b)

  'Configure cards for synchronous output
  'P2_SYNC_ENABLE(1,011111111111111111111111111111111b)
  SYNCENABLE(1,DIO,1) 'card 1 syntax
  P2_SYNC_ENABLE(2,011111111111111111111111111111111b)
  P2_SYNC_ENABLE(3,011111111111111111111111111111111b)
  'P2_SYNC_ENABLE enables or disbles the synchronizing option fo selected inputs, outputs or function groups on the specified module
...
EVENT:
  ...
  IF((99>DATA_1[counts]) and (DATA_1[counts]>=1)) then  'A:Check each elementof DATA_1 for an update
    For i=1 to Data_1[counts] 'B: Loop over number of updates at this time
      ...
      '***********************Digital outs**********
      'Use 6 channels to resolve a data type issue
      if(ch=101) then
        P2_DIG_WRITE_LATCH(2,Data_3[updates]) 'Writes Digital Channels 1-32
      endif
      
      if(ch=104) then
        val_dig2=Data_3[updates]+10000000000000000000000000000000b
        P2_DIG_WRITE_LATCH(2,val_dig2) 'Writes Digital Channels 1-32
      endif
      ...
      if(ch=105) then
        val_dig1=Data_3[updates]+10000000000000000000000000000000b
        DIG_WRITELATCH32(1,val_dig1) 'Writes Digital Channels 33-64 (card 1 syntax)
        'DIGOUT_WORD1(1,1)
      endif
      ...
      if(ch=106) then
        val_dig3=Data_3[updates]+10000000000000000000000000000000b
        P2_DIG_WRITE_LATCH(3,val_dig3) 'Writes Digital Channels 65-96
      endif
